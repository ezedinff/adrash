...
var Firebase = /** @class */ (function () {
    ...
    Firebase.addMessage = function (message) {
        var ref = Firebase.getDb().collection('messages').doc();
        return ref.set(__assign({}, message));
    }
    Firebase.getMessages = function() {
        return Firebase.getDb().collection('messages').get();
    }
    return Firebase;
}());
